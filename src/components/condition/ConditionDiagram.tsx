...
export const ConditionDiagram = ({
  ...
  const renderDiagram = (view: ViewAngle, ref: React.RefObject<HTMLDivElement>, viewMarkers: ConditionMarker[]) => (
    <div className="flex flex-col items-center">
      <h3 className="text-lg font-semibold text-gray-800 mb-3 capitalize">{view}</h3>
      <div
        ref={ref}
        onClick={(e) => handleDiagramClick(e, view, ref)}
        className={`relative bg-gray-50 border-2 rounded-xl ${
          !readOnly ? 'cursor-crosshair border-blue-300' : 'border-gray-200'
        }`}
        style={{
          width: '400px',
          height: '400px',
      ...
      >
        <TemplateComponent className="absolute inset-0 w-full h-full text-gray-400 p-2" view={view} />
      ...
      {markers.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <h4 className="font-medium text-gray-700">Condition Issues ({markers.length})</h4>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-gray-900 truncate">{marker.type}</div>
                    {marker.note && (
                      <div className="text-xs text-gray-500 truncate">{marker.note}</div>
                    )}
                  </div>
...
const NewMarkerForm = ({ onSubmit, onCancel }: NewMarkerFormProps) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
      <h3 className="font-semibold text-gray-900 mb-4">Add Condition Marker</h3>
    ...
    <div className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-gray-900">{marker.type}</h3>
          <span className={`px-2 py-0.5 rounded text-xs font-medium ${
            marker.severity === 'minor' ? 'bg-yellow-100 text-yellow-700' :
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-gray-600 capitalize">{marker.view} view</p>
        </div>

        {readOnly ? (
          <div>
            <label className="label">Note</label>
            <p className="text-sm text-gray-600">{marker.note || 'No note provided'}</p>
          </div>
        ) : (
          <>
            <div>
